---
title: "CHICAGO RATS BE GONE"
output:
  html_document:
    toc: true 
    toc_float: true
    toc_depth: 3 
    code_folding: hide
    css: "assets/css/style.css"
    self-contained: no
    includes: 
      in_header: "assets/html/favicon.html"
---
<p class="subtitle-size text-center"><img class="logo animated infinite pulse" src="assets/images/rat.png" alt="rat">Visualizing Chicago's rats infestation and sanitation problems</p>
<hr class="divider">
```{r error=FALSE, warning=FALSE, results='hide', message=FALSE}
# Libraries
library(lubridate)
library(tidyverse)
library(dplyr)
library(gghighlight)
library(ggrepel)
library(ggthemes)
library(readxl)
library(scales)
library(statar)
library(stringr)
library(tidyr)
library(here)
library(httr)
library(jsonlite)
library(magrittr)
library(grid)
library(ggmap)
library(sf)
library(cowplot)
library(ggplotify)
library(ggplot2)
library(treemapify)
library(showtext)

# Font Setup
font_add_google("Monda", "monda")
showtext_auto()
```

## Emerging Rat Concerns...
In recent years, Chicago has been receiving increasing numbers of rodent infestation complaints. It is important for the city government to actively engage in resolving rat complaints, as rats can cause huge property damage and spread harmful pathogens to humans. This exploratory visualization project illustrates the severity and patterns of rat complaints within the city of Chicago, and seeks to help policymakers to find effective measures to mitigate rodent problems.
```{r cache=TRUE, error=FALSE, warning=FALSE, results='hide', message=FALSE}
# Reading in Data
census_tracts <- 
  read_csv(here("data", 
                "tract_community.csv"))

community_numbers <-
  read_excel(here("data", 
                  "community_numbers.xlsx"))

chicago_rodents <- 
  read_csv(here("data", 
                "Chicago_311_Rodent_2014-2018.csv"))

boston_311 <- 
  read_csv(here("data",
                "Boston_311.csv"))

dc_311_2018 <- 
  read_csv(here("data",
                "DC_Service_Requests_2018.csv"))

nyc_rodents_2018 <- 
  read_csv(here("data",
                "NYC_311_Rodent_2018.csv"))

la_rodents_2018 <- 
  read_csv(here("data",
                "LA_Dead_Animal_Removal_2018.csv"))

detroit_rodents_2018 <- 
  read_csv(here("data",
                "Detroit_Rodent_2018.csv"))

chicago_sanitation_2018 <- 
  read_csv(here("data",
                "Chicago_311_Sanitation_Violations_2018.csv"))

chicago_vacant_property <-
  read_excel(here("data",
                  "Chicago_vacant_properties.xlsx"))

# Scraping Data from ACS
base <- "https://api.census.gov/data/2016/acs/acs5?"
acs_vars <- "get=B01003_001E,B07013_001E,B07013_003E,B19301_001E"
state_tracts <- "&for=tract:*&in=state:17"
ACS_TOKEN <- 
  read_delim(here("data", "ACSAuth.txt"), 
             delim = "/", 
         col_names = F)$X1
key <- paste("&key=", ACS_TOKEN, sep = "")

url <- paste(base, acs_vars, state_tracts, key, sep = "")
response <- GET(url)
all_il_tracts <- 
  jsonlite::fromJSON(content(response, as = "text")) %>%
  data.frame() %>%
  slice(-1)

colnames(all_il_tracts) <- 
  c("Population",
    "Total_Living_In_Area",
    "Renter_Count",
    "Income_Per_Capita",
    "State",
    "County",
    "Tract")
```

## Understand Rat Problems through Graphs
```{r error=FALSE, warning=FALSE, results='hide', message=FALSE}
# Preparing Data
census_tracts <-
  census_tracts %>%
  left_join(community_numbers, 
            by = c("Community Area" = "Number"))

chicago_rodents_2018 <-
  chicago_rodents %>%
  filter(year(mdy(`Creation Date`)) == 2018)

chicago_rodents_2017 <-
  chicago_rodents %>%
  filter(year(mdy(`Creation Date`)) == 2017)

chicago_rodents_2016 <-
  chicago_rodents %>%
  filter(year(mdy(`Creation Date`)) == 2016)

chicago_rodents_2015 <-
  chicago_rodents %>%
  filter(year(mdy(`Creation Date`)) == 2015)

chicago_rodents_2014 <-
  chicago_rodents %>%
  filter(year(mdy(`Creation Date`)) == 2014)
  
boston_rodents_2018 <-
  boston_311 %>%
  filter(str_detect(TYPE, "Rodent"),
         year(open_dt) == 2018)

dc_rodents_2018 <-
  dc_311_2018 %>%
  filter(str_detect(SERVICECODEDESCRIPTION, "Rodent"))

chicago_rat_community_17 <-
  chicago_rodents %>%
  filter(year(mdy(`Creation Date`)) == 2017) %>%
  group_by(`Community Area`) %>%
  summarise(`Number of Complaints` = n()) %>%
  inner_join(community_numbers, by = c("Community Area" = "Number")) %>%
  select(Community, `Number of Complaints`)

to_numeric <- 
  function(column) {
    return(as.numeric(levels(column))[column])
  }

Chicago_communities <- 
  all_il_tracts %>%
  mutate(Census_Tract = str_c(State, County, Tract),
         Total_Income = to_numeric(Income_Per_Capita) * 
                        to_numeric(Population)) %>%
  select(Census_Tract, 
         Population,
         Total_Income,
         Total_Living_In_Area, 
         Renter_Count) %>%
  mutate(Census_Tract = as.numeric(Census_Tract)) %>%
  inner_join(census_tracts, 
             by = c("Census_Tract" = "Census Tract")) %>%
  mutate_at(vars(-"Community",
                 -"Total_Income"), 
            .funs = "to_numeric") %>%
  select(-Census_Tract, -`Community Area`)

Chicago_communities <- 
  Chicago_communities %>%
  group_by(Community) %>%
  summarise_all(.funs = sum) %>%
  mutate(`Income Per Capita` = round(Total_Income / Population, 0),
         `Renter Proportion` = Renter_Count / Total_Living_In_Area) %>%
  select(Community, Population, `Renter Proportion`, `Income Per Capita`)

chicago_rat_community_17 <-
  Chicago_communities %>%
  inner_join(chicago_rat_community_17, by = "Community")

chicago_rat_community_17 %<>%
  mutate(`Number of Requests per 10000 People` = 
           `Number of Complaints` / Population * 10000
         )

# Plot Custom Theme
textcolor <- "#1f2226"

bgcolor <- "linen"

treemap_color <- c("#2f5769", # darkblue
                   "#2e94b5", # blue
                   "#b3a472", # blueorange
                   "#f8b500", # orange
                   "#d6231e", # red
                   "#7a2d24", # darkred
                   "#a86c69", # bluered
                   "#e97718"  # orangered
                   )

custom_theme <-
  theme(
          plot.title = element_text(size = 31,
                                    face = "bold",
                                  family = "monda",
                              lineheight = 0.5,
                                   color = textcolor),
       plot.subtitle = element_text(size = 29,
                                  family = "monda",
                              lineheight = 0.5,
                                   color = textcolor),
        plot.caption = element_text(size = 23,
                                  family = "monda",
                              lineheight = 0.5,
                                   color = textcolor),
         plot.margin =          margin(t = 30, 
                                       r = 30, 
                                       b = 30, 
                                       l = 30, 
                                    unit = "pt"),
          axis.title = element_text(size = 25,
                                    face = "bold",
                                  family = "monda",
                              lineheight = 0.5,
                                   color = textcolor),
           axis.text = element_text(size = 24,
                                    face = "bold",
                                  family = "monda",
                              lineheight = 0.5, 
                                   color = textcolor),
         axis.line.x = element_line(
                                   color = textcolor,
                                    size = 0.7
                                   ), 
   legend.background = element_rect(fill = bgcolor),
        legend.title = element_text(
                                  family = "monda"
                                  ), 
         legend.text = element_text(
                                  family = "monda"
        ),
          legend.key = element_rect(fill = NA, 
                                   color = NA),
  panel.grid.major.x = element_blank(),
  panel.grid.major.y = element_line(size = 0.6, 
                                   color = textcolor,
                                linetype = "dotted"),
  panel.grid.minor.x = element_blank(),
  panel.grid.minor.y = element_blank(),
        panel.border = element_blank(),
    panel.background = element_rect(fill = bgcolor),
     plot.background = element_rect(fill = bgcolor)
  )
```

### "Rat Capital" of United States?
In 2018, we saw a total of 37,577 rat complaints in Chicago. Compared with other major US cities like New York and Los Angeles, and cities that were historically troubled by rodent infestations such as Detroit, Chicago not only had the highest total complaint count, but appeared to have the largest number of complaints per 10,000 population all year round. From the graph below, we can also see that rat complaints were especially prevalent in the summer.
```{r error=FALSE, fig.height=7.5, fig.width=11, message=FALSE, warning=FALSE, fig.showtext=TRUE}
# Data Processing for Plot
city_mon_summary <- 
  function(df, date_col, population, city_name) {
    monthly_summary <-
      df %>%
      group_by(month(date_col, 
                   label = TRUE, 
                    abbr = TRUE)) %>%
      summarise(`Number of Complaints` = n(),
                `Number of Complaints per 10,000 Population` = 
                  `Number of Complaints` / population * 10000,
                                  City = city_name) %>%
      rename(Month = 1)
    return(monthly_summary)
  }

chicago_months <- 
  city_mon_summary(chicago_rodents_2018,
                   mdy(chicago_rodents_2018$`Creation Date`),
                   2716450,
                   "Chicago")

boston_months <- 
  city_mon_summary(boston_rodents_2018,
                   boston_rodents_2018$open_dt,
                   685094,
                   "Boston")

dc_months <- 
  city_mon_summary(dc_rodents_2018,
                   dc_rodents_2018$ADDDATE,
                   693972,
                   "Washington DC")

nyc_months <- 
  city_mon_summary(nyc_rodents_2018,
                   mdy_hms(nyc_rodents_2018$`Created Date`),
                   8622698,
                   "New York City")

la_months <-
  city_mon_summary(la_rodents_2018,
                   mdy_hms(la_rodents_2018$CreatedDate),
                    3999759,
                   "Los Angeles")

detroit_months <- 
  city_mon_summary(detroit_rodents_2018,
                   mdy_hms(detroit_rodents_2018$`Created At`),
                   673104,
                   "Detroit")

city_months_18 <- 
  bind_rows(chicago_months,
            boston_months,
            dc_months,
            nyc_months,
            la_months,
            detroit_months)

# Plot
custom_tt_color <- 
  function(text, color, x, y) {
    annotation_custom(
      grob = textGrob(label = text, 
                      hjust = 0, 
                         gp = gpar(col = color, 
                              fontsize = 31,
                            fontfamily = "monda",
                              fontface = 2)),
      xmin = x, xmax = x,
      ymin = y, ymax = y
    )   
  }

p1 <- city_months_18 %>%
  ggplot(aes(x = Month, 
             y = `Number of Complaints per 10,000 Population`, 
         group = City,
         color = City)) +
  geom_point() +
  geom_line() + 
  geom_line(data = chicago_months,
            size = 1.1) +
  geom_hline(
      yintercept = 0, 
            size = 0.6,
           color = textcolor,
        linetype = "dotted"
      ) +
  labs(
           title = " ",
        subtitle = "Rat complaints per 10,000 population by highly affected US cities in 2018",
         caption = "\nSource: City Open Data Portals"
    ) +
  scale_color_manual(values = c("#2f5769", # darkblue
                                "#d6231e", # red
                                "#a86c69", # bluered
                                "#f8b500", # orange
                                "#2e94b5", # blue
                                "#e97718")) + # orangered
  custom_theme +
  theme(     
             plot.title = element_text(hjust = 0.47,
                                        face = "bold",
                                      family = "monda",
                                  lineheight = 0.5),
          plot.subtitle = element_text(hjust = 0.47),
            legend.text = element_text( size = 27,
                                  lineheight = 1),
            axis.line.y = element_blank(),
      legend.background = element_rect(),
        legend.position = "top",
        legend.box.just = "top",
   legend.justification = "center",
            plot.margin = margin(t = 30,
                                 r = 45, 
                                 l = 30, 
                                 b = 30,
                              unit = "pt")
     ) +
  guides(color = guide_legend(title = NULL)) +
  custom_tt_color("Has the Most Rat Complaints per 10,000 Population All Year Round", textcolor, 2.45, 25.65) +
  custom_tt_color("Chicago", "#d6231e", 1.3, 25.65)

p1 <- ggplot_gtable(ggplot_build(p1))
p1$layout$clip[p1$layout$name == "panel"] <- "off"
grid.newpage()
grid.draw(p1)
```

### When Do We See Most Rat Complaints?
Many homeowners seem to have the misconception that rats will only invade their houses in the winter since it is when temperatures at home differ significantly from the outside. However, in Chicago, we can see from the heatmap that rat complaints were most frequent in the summer, indicating that rats were potentially most rampant at that time (or at least more likely to be seen). Summer is when temperatures are most moderate for rodents to move around and for smell of food to spread. The higher level of human activities may result in greater amount of trash that appeals to rats.
```{r error=FALSE, fig.height=7.5, fig.width=11, message=FALSE, warning=FALSE, fig.showtext=TRUE}
chicago_rodents_year <-
  chicago_rodents %>%
  mutate(Year = year(mdy(`Creation Date`))) %>%
  group_by(Year) %>%
  summarise(year_count = n())

chicago_rodents_mon <-
  chicago_rodents %>%
  mutate(Year = year(mdy(`Creation Date`)),
        Month = month(mdy(`Creation Date`),
                      label = TRUE, 
                       abbr = TRUE)) %>%
  group_by(Year, Month) %>%
  summarise(month_count = n())

chicago_rodents_year_mon <-
  chicago_rodents_year %>%
  inner_join(chicago_rodents_mon, by = "Year") %>%
  mutate(`Percent of Annual Complaints\nEach Month Accounts for` =
           month_count / year_count)

chicago_rodents_year_mon %>%
  ggplot(aes(x = Month,
             y = Year)) +
  geom_tile(aes(
    fill = `Percent of Annual Complaints\nEach Month Accounts for`
    ),
   color = bgcolor) +
  labs(title = "Rat Complaints Are Most Frequent during Summer in Chicago (2014 to 2018)",
    subtitle = "Rats may multiply or become easier to notice due to warm weathers and abundance\nof food created by more human activities in summer.",
     caption = "Source: Chicago Data Portal") +
  scale_y_reverse() +
  scale_fill_continuous(low = "#f8b500",
                       high = "#d6231e", 
                     limits = c(0, 0.15),
                     breaks = c(0, 0.05, 0.1, 0.15),
                     labels = c("0%", "5%", "10%", "15%")) +
  custom_theme +
  theme(
               axis.ticks.x = element_blank(),
                axis.line.x = element_blank(),
               axis.ticks.y = element_blank(),
                axis.line.y = element_blank(),
            legend.position = "top",
           legend.key.width = unit(2.5, "cm"),
       legend.justification = "center",
               legend.title = element_text( size = 25, 
                                            face = "bold",
                                      lineheight = 0.5,
                                           color = textcolor),
         panel.grid.major.y = element_blank(),
                legend.text = element_text( size = 24,
                                      lineheight = 0.5,
                                           color = textcolor),
                 plot.title = element_text(hjust = 0.5),
              plot.subtitle = element_text(hjust = 0.5),
                plot.margin = margin(t = 30,
                                     r = 45, 
                                     l = 30, 
                                     b = 30,
                                  unit = "pt")
  )
```

### Distribution of Sanitation Code Violations and Rat Complaints
Although rat complaint distribution is not equivalent of the actual rat distribution, Chicago Lincoln Park Zoo has recently conducted <a href="https://www.frontiersin.org/articles/10.3389/fevo.2018.00189/full" target="_blank" class="research_link">research</a> indicating that it can be highly reflective of rodent infestation situation. The research also shows that the abundance of rats is potentially related to the abundance of garbage and other types of sanitation problems. That begs the question where rats and garbage were complained frequently and how much they overlapped in different areas of Chicago. As is shown in the map below, we can see that sanitation code violations and rat complaints might have some geographical relationship.
```{r error=FALSE, fig.height=9.95, fig.width=11, message=FALSE, warning=FALSE, fig.showtext=TRUE}
# Data Processing for Plot 
GOOGLE_TOKEN = read_delim(here("data", "GoogleAuth.txt"),
                        delim = "/", 
                    col_names = F)$X1

register_google(GOOGLE_TOKEN)

chicago <- get_map("Chicago", 
                maptype = "toner",
                  scale = 4,
                 source = "stamen")

chicago_rodent_geo <-
  chicago_rodents_2018 %>% 
  group_by(Longitude, Latitude) %>%
  summarise(`Complaint Count` = n()) %>%
  filter(`Complaint Count` > 6, !is.na(Longitude)) %>%
  mutate(`Complaint Type` = "Rat Complaints\n(>6 Times)")

chicago_sanitation_geo <-
  chicago_sanitation_2018 %>%
  group_by(Longitude, Latitude) %>%
  summarise(`Complaint Count` = n()) %>%
  filter(`Complaint Count` > 3, !is.na(Longitude)) %>%
  mutate(`Complaint Type` = "Sanitation Code\nViolations\n(>3 Times)")

chicago_geo <- bind_rows(chicago_rodent_geo,
                         chicago_sanitation_geo)

# Plot 
ggmap(chicago) +
  geom_point(aes(x = `Longitude`,
                 y = `Latitude`,
             color = `Complaint Type`,
              size = `Complaint Count`),
             alpha = 0.8,
              data = chicago_geo) +
  labs(title = "Rat Complaints Were Generally Located near\nPlaces with Sanitation Concerns (2018)",
    subtitle = "The north of Chicago has slightly more complaints than the south",
     caption = "Source: Chicago Data Portal",
           x = "Longitude",
           y = "Latitude") +
  scale_x_continuous(limits = c(-88.13, -87.23), expand = c(0, 0)) +
  scale_y_continuous(limits = c(41.58, 42.1), expand = c(0, 0)) +
  scale_color_manual(values = c("#d6231e", #red
                                "#2e94b5" # blue
                                )) + 
  custom_theme +
  theme(legend.justification = "center",
                  plot.title = element_text(hjust = 0.6),
               plot.subtitle = element_text(hjust = 0.6),
                 plot.margin = margin(t = 30,
                                      r = 45, 
                                      l = 0, 
                                      b = 30,
                                   unit = "pt"),
                legend.title = element_text( size = 25, 
                                             face = "bold",
                                           family = "monda",
                                            color = textcolor),
                 legend.text = element_text( size = 22,
                                           family = "monda",
                                       lineheight = 0.4,
                                            color = textcolor),
            legend.key.width = unit(0.25, "cm"),
             legend.position = c(.8, .7),
          panel.grid.major.y = element_blank(),
                axis.title.x = element_blank(),
                axis.title.y = element_blank(),
                 axis.text.x = element_blank(),
                 axis.text.y = element_blank(),
                 axis.line.x = element_blank(),
                 axis.line.y = element_blank(),
                axis.ticks.x = element_blank(),
                axis.ticks.y = element_blank()) +
  guides(color = guide_legend(keyheight = 2,
                            title.vjust = -2.5))
```

### Surrounding Premises Where Rat Complaints Are Filed
From the map above, we can see that rat complaints were generally distributed near sanitation code violations. If we take a further look at the premises where rat complaints were filed, we can see that most complaints were from premises associated with unhandled garbage from 2014 to 2018. Indeed, acting as potential food sources, garbage is most appealing to rodents. It is important for the city government to make sure all properties empty garbage with a timely manner.
```{r error=FALSE, fig.height=10, fig.width=11, message=FALSE, warning=FALSE, fig.showtext=TRUE}
premise_indicators1 <- 
  chicago_rodents %>%
  filter(`Number of Premises Baited` > 0,
         `Number of Premises with Garbage` > 0,
         `Number of Premises with Rats` > 0) %>%
  mutate(`Surrounding Premises with Indicators` = "Premises with\nTraps, Garbage & Rats\n126,003")

premise_indicators2 <- 
  chicago_rodents %>%
  filter(`Number of Premises Baited` > 0,
         `Number of Premises with Garbage` > 0,
         `Number of Premises with Rats` == 0) %>%
  mutate(`Surrounding Premises with Indicators` = "Traps &\nGarbage\n3,989")

premise_indicators3 <- 
  chicago_rodents %>%
  filter(`Number of Premises Baited` == 0,
         `Number of Premises with Garbage` > 0,
         `Number of Premises with Rats` > 0) %>%
  mutate(`Surrounding Premises with Indicators` = "Garbage & Rats\n13,766")

premise_indicators4 <- 
  chicago_rodents %>%
  filter(`Number of Premises Baited` > 0,
         `Number of Premises with Garbage` == 0,
         `Number of Premises with Rats` > 0) %>%
  mutate(`Surrounding Premises with Indicators` = "Traps & Rats\n13,935")

premise_indicators5 <- 
  chicago_rodents %>%
  filter(`Number of Premises Baited` > 0,
         `Number of Premises with Garbage` == 0,
         `Number of Premises with Rats` == 0) %>%
    mutate(`Surrounding Premises with Indicators` = "Only\nTraps\n2,071")

premise_indicators6 <- 
  chicago_rodents %>%
  filter(`Number of Premises Baited` == 0,
         `Number of Premises with Garbage` > 0,
         `Number of Premises with Rats` == 0) %>%
  mutate(`Surrounding Premises with Indicators` = "Only Garbage\n17,601")

premise_indicators7 <- 
  chicago_rodents %>%
  filter(`Number of Premises Baited` == 0,
         `Number of Premises with Garbage` == 0,
         `Number of Premises with Rats` > 0) %>%
  mutate(`Surrounding Premises with Indicators` = "Only Rats\n1,621")

premise_indicators8 <- 
  chicago_rodents %>%
  filter(`Number of Premises Baited` == 0,
         `Number of Premises with Garbage` == 0,
         `Number of Premises with Rats` == 0) %>%
  mutate(`Surrounding Premises with Indicators` = "None\n15,912")

premise_indicators <- 
  bind_rows(premise_indicators1,
            premise_indicators2,
            premise_indicators3,
            premise_indicators4,
            premise_indicators5,
            premise_indicators6,
            premise_indicators7,
            premise_indicators8)

premise_indicators %>%
  group_by(`Surrounding Premises with Indicators`) %>%
  summarise(Count = n()) %>%
  ggplot() +
  geom_treemap(aes(area = Count,
                  label = `Surrounding Premises with Indicators`,
                   fill = factor(Count)),
                  color = bgcolor) +
  geom_treemap_text(aes(area = Count,
                       label = `Surrounding Premises with Indicators`,
                        fill = factor(Count)),
                       color = bgcolor,
                       place = "center",
                        size = 27,
                      family = "monda",
                  lineheight = 0.5) +
  scale_fill_manual(values = treemap_color) +
  labs(
          title = "Most Rat Complaints Are Filed at Baited Premises with Garbage and Rats (2014 to 2018)",
       subtitle = "Premises with garbage seem to be more related with rat complaints in Chicago",
        caption = "Source: Chicago Data Portal"
       ) +
  custom_theme +
  theme(legend.position = "none",
          plot.subtitle = element_text(hjust = 0.5),
            plot.margin = margin(t = 30,
                                 r = 50, 
                                 l = 50, 
                                 b = 30,
                              unit = "pt"))
```

### Rats and Empty Homes
Rats, being the ultimate squatters, see vacant houses as their prime real estate. Such places offer hassle-free shelter; nooks and crannies for hiding and playing; carpeting for nests; and toilet bowls for swimming. The best houses for rodents are those that have been vacant for a while or haven’t yet been fixed up, as these often provide a convenient water supply via leaks or other accumulated moisture. Vacant Properties that are foreclosed generally entail high risks, and once a single unit has been infested, it is only a matter of time before rats venture out into the neighborhood in search of food and new places to inhabit. The plot below shows that communities with more vacant properties are more likely to attract rats.
```{r error=FALSE, fig.height=7.5, fig.width=11, message=FALSE, warning=FALSE, fig.showtext=TRUE}
# Data Processing for Plot 
property_rat <- 
  function(data) {
    chicago_rat_community <-
      data %>%
      group_by(`Community Area`) %>%
      summarise(`Number of Complaints` = n()) %>%
      inner_join(community_numbers, by = c("Community Area" = "Number")) %>%
      select(Community, `Number of Complaints`)

    chicago_rat_property <-
      chicago_rat_community %>%
      inner_join(chicago_vacant_property, by = "Community")
    
    return(chicago_rat_property)
  }

chicago_rat_property_14 <- property_rat(chicago_rodents_2014)
chicago_rat_property_15 <- property_rat(chicago_rodents_2015)
chicago_rat_property_16 <- property_rat(chicago_rodents_2016)
chicago_rat_property_17 <- property_rat(chicago_rodents_2017)
chicago_rat_property_18 <- property_rat(chicago_rodents_2018)

chicago_rat_property <- bind_rows(chicago_rat_property_14,
                                  chicago_rat_property_15,
                                  chicago_rat_property_16,
                                  chicago_rat_property_17,
                                  chicago_rat_property_18)

# Plot 
chicago_rat_property %>%
  ggplot(aes(x = `Vacant Housing Units`,
             y = `Number of Complaints`)) +
  geom_hex(color = bgcolor) +
  labs(
       title = "Communities with More Vacant Properties Are More Likely to\nSee Rodent Complaints in Chicago (2014 to 2018)",
    subtitle = "Perfect insulation in the walls for nesting may be what is attracting rodents",
     caption = "Source: American Community Survey & Chicago Data Portal",
           x = "Number of Vacant Housing Units"
  ) +
  scale_fill_continuous("Community\nCount", 
                        low = "#f8b500",
                       high = "#d6231e") +
  scale_x_continuous(labels = scales::comma, 
                     limits = c(0, 3850)) +
  scale_y_continuous(labels = scales::comma) +
  custom_theme +
  theme(
    plot.subtitle = element_text(hjust = 0.5),
     plot.caption = element_text(hjust = 1.37),
      axis.line.y = element_blank(),
     legend.title = element_text(size = 25,
                           lineheight = 0.5,
                                 face = "bold",
                                color = textcolor),
      legend.text = element_text(size = 24,
                           lineheight = 0.5,
                                color = textcolor)
  )
```

### How Has Response Time Changed Over Years?
The city has become increasingly responsive to rat complaints. In 2016, the mayor's absolute goal for city workers was to respond to all relevant complaints within 5 days, and now it seems that this goal is largely achieved. The bar chart below shows how days to resolve complaints have been decreasing throughout different months of years. Starting from 2016, response time has dropped drastically, but the number of total rat complaints seems unchanged.
```{r error=FALSE, fig.height=9, fig.width=11, message=FALSE, warning=FALSE, fig.showtext=TRUE}
# Plot 
chicago_rodents %>%
  filter(Status == "Completed") %>%
  mutate(Resolving_Days = as.numeric(mdy(`Completion Date`)) - 
                          as.numeric(mdy(`Creation Date`)),
         `Days to Resolve\nRat Complaints` = fct_collapse(
           factor(Resolving_Days),
           "within 3 days" = as.character(0:3),
             "4 to 7 days" = as.character(4:7),
            "8 to 14 days" = as.character(8:14),
           "15 to 30 days" = as.character(15:30),
            "over 30 days" = as.character(31:500)
         ),
          year = year(mdy(`Creation Date`)),
         month = month(mdy(`Creation Date`), 
                       label = TRUE, 
                        abbr = TRUE)
         ) %>%
  group_by(year, month) %>%
  ggplot() +
  geom_bar(aes(x = month, 
            fill = `Days to Resolve\nRat Complaints`),
        position = position_stack(reverse = TRUE)) +
  facet_wrap(vars(year), nrow = 5) +
  labs(title = "Rat Complaint Response Time Has Dropped over\nthe Past Few Years in Chicago (2014 to 2018)",
    subtitle = "Since 2017, most rat complaints are resolved within 7 days",
     caption = "Source: Chicago Data Portal",
           x = "Month",
           y = "Number of Resolved Complaints") +
    scale_fill_manual(values = c("#2e94b5",  # blue
                                 "#b3a472",  # blueorange
                                 "#f8b500",  # orange
                                 "#e97718",  # orangered
                                 "#d6231e"), # red
                      "Days to Resolve\nRat Complaints") +
  scale_y_continuous(labels = scales::comma) +
  custom_theme +
  theme(plot.title = element_text(hjust = 0.55),
     plot.subtitle = element_text(hjust = 0.55),
      plot.caption = element_text(hjust = 1.28),
       legend.text = element_text( size = 20,
                             lineheight = 0.5,
                                  color = textcolor),
      legend.title = element_text( size = 22, 
                                   face = "bold",
                             lineheight = 0.5,
                                  color = textcolor),
        strip.text = element_text( face = "bold",
                                   size = 22,
                                 family = "monda",
                             lineheight = 0.5,
                                  color = textcolor),
       axis.line.y = element_blank(),
  strip.background = element_blank())
```

### Response Time Reduced for All Communities
From 2014 to 2018, Chicago's overall response time to rat complaints have been steadily decreasing for all seasons. Besides seasonality analysis, looking at response time changes at a community level may also help policymakers more effectively target potential rat issues in the future. The map set below shows that the irresponsiveness problem appeared most severe in 2015, especially in the west of Chicago, and shortly after, almost all Chicago communities became much more responsive to rat complaints. 
```{r error=FALSE, fig.height=10.25, fig.width=11, message=FALSE, warning=FALSE, fig.showtext=TRUE}
# Data Processing and Mapping Preparation
cowplot_theme <-
  theme(panel.background = element_rect(fill = bgcolor),
         plot.background = element_rect(fill = bgcolor),
       legend.background = element_rect(fill = bgcolor),
        panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
            panel.border = element_blank())

create_response <- 
  function(year_data) {
    result <- 
      year_data %>%
      filter(Status == "Completed") %>%
      mutate(Resolving_Days = as.numeric(mdy(`Completion Date`)) - 
                              as.numeric(mdy(`Creation Date`))) %>%
      group_by(`Community Area`) %>%
      summarise(avg_resolving_days = ceiling(mean(Resolving_Days))) %>%
      mutate(`Community Area` = as.character(`Community Area`),
            `Days to Resolve Rat Complaints` =
              fct_collapse(factor(avg_resolving_days),
               "within 3 days" = as.character(0:3),
                 "4 to 7 days" = as.character(4:7),
                "8 to 14 days" = as.character(8:14),
               "15 to 30 days" = as.character(15:30),
                "over 30 days" = as.character(31:500))) %>%
      filter(!is.na(`Community Area`))
    return(result)
  }

response_2014 <- create_response(chicago_rodents_2014)
response_2015 <- create_response(chicago_rodents_2015)
response_2016 <- create_response(chicago_rodents_2016)
response_2017 <- create_response(chicago_rodents_2017)
response_2018 <- create_response(chicago_rodents_2018)

chicago_shp <- 
  read_sf(here("data",
               "geo_export_b4a17cee-2ce2-4bde-9c91-82d509e2a7c9.shp"))

create_res_plot <- 
  function(year_response, year) {
    year_map <- 
      chicago_shp %>%
      left_join(year_response, by = c("area_num_1" = "Community Area"))
    if(year == 2014) {
      scale_map <- 
        scale_fill_manual(values = c("#f8b500", # 3
                                     "#e97718", # 4
                                     "#d6231e"  # 5
                                     ))
    } else if (year == 2015) {
      scale_map <-
        scale_fill_manual(values = c("#b3a472", # 2
                                     "#e97718", # 4
                                     "#d6231e"  # 5
                                     ))
    } else if (year == 2016) {
      scale_map <-
        scale_fill_manual(values = c("#b3a472", # 2
                                     "#f8b500"  # 3
                                     ))
    } else if (year == 2017) {
      scale_map <- 
        scale_fill_manual(values = c("#2e94b5", # 1
                                     "#b3a472"  # 2
                                     ))
    } else if (year == 2018) {
      scale_map <- 
        scale_fill_manual(values = c("#2e94b5", # 1
                                     "#b3a472"  # 2
                                     ))
    }
    return (
      ggplot(year_map) +
        geom_sf(aes(fill = `Days to Resolve Rat Complaints`)) +
        coord_sf(  datum = NA) +
        background_grid(minor = "none") +
        scale_map +
        cowplot_theme +
        theme(legend.position = "none")
    )
  }

# Plot
map_14 <- create_res_plot(response_2014, 2014)
map_15 <- create_res_plot(response_2015, 2015)
map_16 <- create_res_plot(response_2016, 2016)
map_17 <- create_res_plot(response_2017, 2017)
map_18 <- create_res_plot(response_2018, 2018)

legend_response <-
  response_2014 %>%
  mutate(`Days to Resolve Rat Complaints` = as.character(`Days to Resolve Rat Complaints`))
  
legend_response[[1,3]] <- "within 3 days"
legend_response[[3,3]] <- "4 to 7 days"

map_legend <-
  chicago_shp %>%
  left_join(legend_response, by = c("area_num_1" = "Community Area")) %>%
  ggplot() +
  geom_sf(aes(fill = `Days to Resolve Rat Complaints`)) +
  coord_sf(datum = NA) +
  cowplot_theme +
  theme(legend.text = element_text(size = 24,
                             lineheight = 0.5,
                                 family = "monda",
                                  color = textcolor),
       legend.title = element_text(size = 25, 
                                   face = "bold",
                                 family = "monda",
                             lineheight = 0.5,
                                  color = textcolor),
    legend.position = c(0.1, 0.5)) +
  scale_fill_manual(values = c("#e97718", # 4
                               "#b3a472", # 2
                               "#f8b500", # 3
                               "#d6231e", # 5
                               "#2e94b5"  # 1
                               ),
                    breaks = c("within 3 days",
                               "4 to 7 days",
                               "8 to 14 days",
                               "15 to 30 days",
                               "over 30 days")) +
   background_grid(major = "none", minor = "none") 

legend <- get_legend(map_legend)

maps <-
  plot_grid(map_14, 
            map_15, 
            map_16,
            legend,
            map_17,
            map_18,
            labels = c("2014", "2015", "2016", " ", "2017", "2018"), 
            label_size = 25,
            label_fontfamily = "monda",
            label_fontface = "bold",
            label_colour = textcolor,
            ncol = 3) +
  background_grid(major = "none", minor = "none") +
  cowplot_theme +
  theme(plot.background = element_rect(color = bgcolor, size = 2))

maps <- as.ggplot(maps)
maps +
  labs(title = "All Chicago Communities Have Reduced Average\nResponse Time to Rat Complaints Over Years",
    subtitle = "Response time to rat complaints peaked at 2015 especially around\ncommunities in the west, and then dropped sharply",
     caption = "Source: Chicago Data Portal") +
  custom_theme +
  theme(
                axis.title.x = element_blank(),
                 axis.text.x = element_blank(),
                axis.ticks.x = element_blank(),
                axis.title.y = element_blank(),
                axis.text.y  = element_blank(),
                axis.ticks.y = element_blank(),
                 axis.line.x = element_line(color = bgcolor, size = 1),
                 axis.line.y = element_line(color = bgcolor, size = 1),
          axis.line.x.bottom = element_line(color = bgcolor, size = 1),
            axis.line.y.left = element_line(color = bgcolor, size = 1),
             plot.background = element_rect( fill = bgcolor),
                  plot.title = element_text(hjust = 0.5,
                                            vjust = 11.5),
               plot.subtitle = element_text(hjust = 0.5,
                                            vjust = 10)
    )
```

### Total Number of Rat Complaints Dropped Over Years?
Although Chicago has increased the complaint response efficiency during the past few years, many of its communities have not effectively reduced the number of rat complaints. There are even some communities that have seen an increase of over 100% of total rat complaints. Certainly, the rats are not gone and also appear to be more rampant in some areas. It is important for the city of Chicago to look into the areas that have gotten worse and reflect on the effectiveness of response methods and strategies.
```{r error=FALSE, fig.height=9.9, fig.width=12, message=FALSE, warning=FALSE, fig.showtext=TRUE}
count14 <- 
  chicago_rodents_2014 %>%
  group_by(`Community Area`) %>%
  summarise(Count14 = n())

count18 <-
  chicago_rodents_2018 %>%
  group_by(`Community Area`) %>%
  summarise(Count18 = n())

count1418 <- 
  count14 %>%
  left_join(count18, by = "Community Area") %>%
  filter(`Community Area` != 0) %>%
  left_join(community_numbers, by = c("Community Area" = "Number")) %>%
  mutate(propChange = (Count18 - Count14)/Count14,
            propBin = cut(propChange,
                          breaks = c(-Inf, 
                                     -0.25,
                                     0, 
                                     0.25, 
                                     0.5, 
                                     0.75, 
                                     1, 
                                     Inf),
                          labels = c("- 25+%",
                                     "- 0-25%",
                                     "+ 0-25%",
                                     "+ 25-50%",
                                     "+ 50-75%",
                                     "+ 75-100%",
                                     "+ 100+%")),
         labelcolor = ifelse(propBin %in% c("- 0-25%", 
                                            "+ 0-25%",
                                            "+ 25-50%"),
                             "dark",
                             "light"))

count1418 %>%
  group_by(`propBin`) %>%
  mutate(CountProp = 1) %>%
  ggplot(aes(x = propBin, 
             y = CountProp)) +
  geom_bar(aes(  fill = propBin), 
                 stat = "identity", 
                color = bgcolor) +
  geom_text(aes(label = `Community`,
               family = "monda",
                color = labelcolor), 
             position = position_stack(vjust = 0.5),
                 size = 7,
                 face = "bold") +
  labs(title = "Despite Shorter 311 Response Time, Most Chicago Communities Have Seen\nIncrease in Total Rat Complaints (2014 to 2018)",
    subtitle = "Many communities have seen rat complaints grow by 100% from 2014 to 2018,\nand decrease in total rat complaints are not prominent.",
           x = "Percentage Change in Rat Complaints from 2014 to 2018",
     caption = "Source: Chicago Data Portal") +
  custom_theme +
  theme(panel.grid.major.y = element_blank(),
              axis.title.y = element_blank(),
               axis.text.y = element_blank(),
               axis.line.y = element_blank(),
              axis.ticks.y = element_blank(),
             plot.subtitle = element_text(hjust = 0.5),
           legend.position = "none") +
  scale_fill_manual(values = c("#2f5769",  # darkblue
                               "#2e94b5",  # blue
                               "#b3a472",  # blueorange
                               "#f8b500",  # orange
                               "#e97718",  # orangered
                               "#d6231e",  # red 
                               "#7a2d24"), # darkred
                    "") +
  scale_color_manual(values = c(textcolor,
                                bgcolor),
                     "") +
  scale_y_continuous(expand = c(0, 0))
```

### Income and Rat Complaints
If we look at the maps of income level and rat complaints in different Chicago communities, we can easily see that most of the areas that have reduced the rat complaints in 2018 are areas in the north and with higher income per capita. However, in the south of Chicago where per capita income is generally lower, many communities have seen increase in rat complaints despite 311 response being faster. It may be a good idea to enhance the quality of 311 response service in lower income areas.
```{r error=FALSE, fig.height=6.2, fig.width=7.8, message=FALSE, warning=FALSE, fig.showtext=TRUE}
complaint_community_18 <-
  chicago_rodents_2018 %>%
  filter(!is.na(`Community Area`)) %>%
  mutate(`Community Area` = as.character(`Community Area`)) %>%
  group_by(`Community Area`) %>%
  summarise(`2018\nComplaint\nCount` = n())

complaint_community_map <- 
  chicago_shp %>%
  left_join(complaint_community_18, 
            by = c("area_num_1" = "Community Area"))

c1 <- 
  ggplot(complaint_community_map) +
  geom_sf(aes(fill = `2018\nComplaint\nCount`)) +
  coord_sf(  datum = NA) +
  custom_theme +
  scale_fill_continuous(low = bgcolor, 
                       high = "#f8b500",
                     labels = scales::comma) +
  theme(
         legend.title = element_text(face = "bold",
                                     size = 16),
      legend.position = "top",
     legend.key.width = unit(1, "cm"),
          plot.margin = margin(t = 0, 
                               r = 0, 
                               b = 0, 
                               l = 0, 
                            unit = "pt")
  ) + 
  guides(
    fill = guide_colorbar(title.position = "top",
                             title.hjust = 0.5)
  )

count1418_map <-
  chicago_shp %>%
  left_join(count1418 %>%
              mutate(`Community Area` = as.character(`Community Area`)),
                  by = c("area_num_1" = "Community Area"))

c2 <-
  ggplot(count1418_map) +
  geom_sf(aes(fill = propBin)) +
  coord_sf(  datum = NA) +
  custom_theme +
  scale_fill_manual(values = c('#faf0e6',
                               '#fbd2c2',
                               '#f9b4a0',
                               '#f4947e',
                               '#ec745d',
                               '#e2513d',
                               '#d6231e'), 
                    "Complaint Percent Change\n     (2014 to 2018)") +
  theme(
        legend.title = element_text(face = "bold",
                                    size = 16),
       legend.margin = margin(l = unit(25, "cm")),
    legend.key.width = unit(0.85, "cm"),
     legend.position = "top",
         plot.margin = margin(t = 0, 
                              r = 0, 
                              b = 0, 
                              l = 0, 
                           unit = "pt")
  ) +
  guides(
    fill = guide_legend(title.position = "top",
                           title.hjust = 0.3,
                                  nrow = 4)
  )

income_community <- 
  community_numbers %>%
  left_join(Chicago_communities, by = "Community") %>%
  mutate(`Average\nIncome\nPer Capita` = `Income Per Capita`)

income_map <-
  chicago_shp %>%
  left_join(income_community %>%
              mutate(Number = as.character(Number)),
            by = c("area_num_1" = "Number"))

c3 <-
  ggplot(income_map) +
  geom_sf(aes(fill = `Average\nIncome\nPer Capita`)) +
  coord_sf(  datum = NA) +
  scale_fill_continuous(low = bgcolor, 
                       high = "#2e94b5",
                     labels = scales::comma) +
  custom_theme +
  theme(
        legend.title = element_text(face = "bold",
                                    size = 16),
     legend.position = "top",
    legend.key.width = unit(1, "cm"),
         plot.margin = margin(t = 0, 
                              r = 0, 
                              b = 0, 
                              l = 0, 
                           unit = "pt")
  ) +
  guides(
    fill = guide_colorbar(title.position = "top",
                             title.hjust = 0.5)
  )
comparison <- 
  plot_grid(c1, c2, c3, ncol = 3)

comparison_gg <- as.ggplot(comparison)

comparison_gg +
  labs(title = "Richer Communities Have Seen Decrease in Rat Complaints\nWhereas Poorer Communities Have Seen Sharp Increase (2014 to 2018)",
    subtitle = "Poorer communities with many complaints in the south and\nwest have not seen improvement over the years",
     caption = "Source: American Community Survey & Chicago Data Portal") +
  custom_theme +
  theme(
              axis.title.x = element_blank(),
               axis.text.x = element_blank(),
              axis.ticks.x = element_blank(),
              axis.title.y = element_blank(),
              axis.text.y  = element_blank(),
              axis.ticks.y = element_blank(),
           plot.background = element_rect( fill = bgcolor),
                plot.title = element_text( size = 20.5,
                                          hjust = 0.5,
                                          vjust = 11.5),
             plot.subtitle = element_text( size = 19,
                                          hjust = 0.5,
                                          vjust = 10),
              plot.caption = element_text( size = 14, 
                                          hjust = 1)
  )
```

<hr class="divider"/>
<p class="text-center"><a href="https://github.com/heguanelvis/Chicago-Rats-Be-Gone">Chicago Rats Be Gone</a></p>
<p class="text-center">Created by <a href="https://github.com/heguanelvis">Guan He</a></p>
<p class="text-center"><span>guanh@uchicago.edu</span></p>
<p class="text-center"><a href="https://github.com/heguanelvis"><i class="fab fa-github-square"></i></a></p>